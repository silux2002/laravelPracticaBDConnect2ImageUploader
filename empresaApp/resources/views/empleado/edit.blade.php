            ...
            <div class="card z-index-0 fadeIn3 fadeInBottom">
              <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                <div class="bg-gradient-primary shadow-primary border-radius-lg py-3 pe-1">
                  <h4 class="text-white font-weight-bolder text-center mt-1 mb-0">Edición de Empleado</h4>
                </div>
              </div>
              <div class="card-body">
                @if ($errors->any()) 
                ...
                <form method="post" action="{{ route('empleados.update', $empleado->id) }}">
                  ...
                  <div class="input-group input-group-static my-3"> 
                      ...
                      <input type="date" class="form-control" name="fecha_contratacion" value="{{ $empleado->fecha_contratacion }}"/> 
                  </div> 
                  <div class="input-group input-group-static mb-4">
                      <label for="phone" class="ms-0">Departamento</label> 
                      <select name="departamento_id" class="form-control">
                          @foreach(\App\Models\Departamento::all() as $d)
                            @if($d->id == $empleado->departamento_id)
                                <option value="{{$d->id}}" name="departamento_id" selected>{{$d->nombredepartamento}} - {{$d->localizacion}}</option>
                            @else
                                <option value="{{$d->id}}" name="departamento_id">{{$d->nombredepartamento}} - {{$d->localizacion}}</option>
                            @endif
                          @endforeach
                      </select>
                  </div>
                  <div class="input-group input-group-static mb-4">
                      ...
                      <select name="puesto_trabajo_id" class="form-control">
                          @foreach(\App\Models\PuestoTrabajo::all() as $d)
                            @if($d->id == $empleado->puesto_trabajo_id)
                                <option value="{{$d->id}}" name="puesto_trabajo_id" selected>{{$d->nombrepuesto}}</option>
                            @else
